        ...
        <v-row>
            <v-col cols="12" sm="4" class="d-flex align-center justify-center">
                <div v-if="playerStore.player === 'embed'" class="video-container">
                    <iframe
                        :src="`https://www.youtube-nocookie.com/embed/${data.id}?autoplay=1&modestbranding=1&enablejsapi=1`"
                        id="youtubeiframechild" frameborder="0"
                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                        allowfullscreen></iframe>
                </div>

                <YTCommonPlayer v-else-if="playerStore.player === 'shaka-player'" ref="child" :videoId="data.id"
                    :key="data.id" @errors="handleError" />

                <YTCommonVideoJS v-else-if="playerStore.player === 'VideoJS'" ref="child" :videoId="data.id"
                    :key="data.id + 'JS'" @errors="handleError" />
            </v-col>
            <v-col cols="12" sm="8">
