<template>
    <v-card v-for="post in posts" :key="post.id" class="mt-4" density="compact">
        ...
        <v-card-actions>
            <v-btn :color="post.likes.includes(auth.currentUser?.uid) ? 'pink' : 'primary'"
                @click="() => $emit('like-post', post.id)">
                <v-icon>{{ post.likes.includes(auth.currentUser?.uid) ? 'mdi-heart' : 'mdi-heart-outline' }}</v-icon>
                Like ({{ post.likes.length }})
            </v-btn>
            <v-btn v-if="post.authorId === auth.currentUser?.uid" @click="() => $emit('delete-post', post.id)"
                color="error" class="ml-auto">Delete</v-btn>
        </v-card-actions>
    </v-card>
</template>
