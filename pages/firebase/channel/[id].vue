  ...
  <v-container>
      ...
      <v-col cols="12" md="8">
        <v-card v-if="user">
          <v-card-title>
            ...
            <v-btn
              v-if="isCurrentUser" color="primary" class="rounded-pill mx-2"
              @click="showEditDialog">編集</v-btn>
          </v-card-title>
    ...
    <v-dialog v-model="isEditDialogVisible" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-form ref="editForm">
            <v-text-field v-model="NewUserName" label="新しい名前" required />
            <v-textarea v-model="NewUserDescription" label="新しい説明" required />
          </v-form>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
