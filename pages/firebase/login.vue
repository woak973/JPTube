<template>
    <v-container v-if="isServiceWorkerRegistered">
        <v-form @submit.prevent="login">
            ...
            <v-btn @click="openResetDialog" color="warning" class="rounded-pill mx-2">パスワードを忘れた場合</v-btn>
        </v-form>
        <v-form @submit.prevent="register" class="mt-4">
            <v-text-field v-model="newEmail" label="New Email" required></v-text-field>
            <v-text-field v-model="newPassword" :type="showNewPassword ? 'text' : 'password'" label="New Password"
                required append-icon="mdi-eye" @click:append="toggleNewPasswordVisibility"></v-text-field>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-form @submit.prevent="sendPasswordReset">
                        <v-text-field v-model="resetEmail" label="Email" required></v-text-field>
