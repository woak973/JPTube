  ...
  <v-container>
    <v-row justify="center">
      <v-col cols="12" md="8">
        <v-card>
          ...
          <v-card-text>
            <v-btn color="primary" class="rounded-pill mx-2" @click="logout">ログアウト</v-btn>
            <v-btn
              :to="`/firebase/channel/${userId}`" color="primary"
              class="rounded-pill mx-2">プロフィール</v-btn>
            <v-btn :to="'/firebase/'" color="primary" class="rounded-pill mx-2">タイムラインを見る</v-btn>
          </v-card-text>
          <v-card-title>タグ:{{ route.params.q }}</v-card-title>
          <v-card-text>
            <v-textarea v-model="postContent" label="New Post" outlined auto-grow rows="1" />
            <div>
              <v-chip v-for="tag in selectedTags" :key="tag" class="ma-2" outlined>{{ tag }}</v-chip>
            </div>
            <v-btn class="rounded-pill mx-2" color="primary" @click="submitPost">投稿</v-btn>
            <v-btn class="rounded-pill mx-2" color="primary" @click="openTagDialog">タグを追加</v-btn>
          </v-card-text>
        </v-card>
    ...
    <v-dialog v-model="tagDialog" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-checkbox
            v-for="tag in availableTags" :key="tag" v-model="selectedTags" :label="tag" :value="tag"
            :disabled="tag === route.params.q" />
          <v-text-field
            v-model="newTag" label="カスタムタグを追加" append-icon="mdi-send" @keyup.enter="addCustomTag"
...
const selectedTags = ref<Array<string>>([route.params.q as string]);
...
const fetchPosts = async () => {
  if (route.params.q === 'タイムラインに載せない') {
    router.push('/firebase/');
    return;
  }
  const q = query(
    collection(db, 'posts'),
    where('tags', 'array-contains', route.params.q as string),
