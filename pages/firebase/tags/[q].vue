    ...
    <v-container>
        ...
        <v-dialog v-model="tagDialog" max-width="500px">
            <v-card>
                ...
                <v-card-text>
                    <v-checkbox v-for="tag in availableTags" :key="tag" :label="tag" :value="tag" v-model="selectedTags"
                        :disabled="tag === route.params.q"></v-checkbox>
                    <v-text-field v-model="newTag" label="カスタムタグを追加" @keyup.enter="addCustomTag" append-icon="mdi-send"
                        @click:append="addCustomTag"></v-text-field>
                </v-card-text>
                <v-card-actions>
