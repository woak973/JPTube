  ...
  <v-container>
    ...
    <v-dialog v-model="tagDialog" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-checkbox
            v-for="tag in availableTags" :key="tag" v-model="selectedTags" :label="tag"
            :value="tag" />
          <v-text-field v-model="newTag" label="カスタムタグを追加" @keyup.enter="addCustomTag" />
        </v-card-text>
        <v-card-actions>
...
useHead({
  title: 'Timeline - JPTube Firebase',
});
